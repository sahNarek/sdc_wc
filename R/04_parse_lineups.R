#' Parse lineups JSON into long-format dataframe
parse_lineups_json <- function(lineups, match_id) {
  if (length(lineups) == 0) {
    return(tibble())
  }

  rows <- lapply(lineups, function(team_block) {
    team_id <- as.integer(team_block$team_id)
    team_name <- as.character(team_block$team_name)

    lapply(team_block$lineup, function(player_row) {
      positions <- player_row$positions
      primary_position <- if (length(positions) > 0) {
        as.character(positions[[1]]$position)
      } else {
        NA_character_
      }

      tibble(
        match_id = as.integer(match_id),
        team_id = team_id,
        team_name = team_name,
        player_id = as.integer(player_row$player_id),
        player_name = as.character(player_row$player_name),
        player_nickname = as.character(player_row$player_nickname),
        jersey_number = as.integer(player_row$jersey_number),
        primary_position = primary_position,
        goals = as.integer(player_row$stats$goals),
        assists = as.integer(player_row$stats$assists)
      )
    })
  })

  bind_rows(unlist(rows, recursive = FALSE))
}

#' Build deduplicated player lookup table
build_player_lookup <- function(lineups_df, events_df = NULL) {
  players <- lineups_df %>%
    transmute(
      player_id,
      player_name,
      player_nickname,
      team_id,
      team_name
    ) %>%
    distinct(player_id, .keep_all = TRUE) %>%
    mutate(
      player_display_name = if_else(
        !is.na(player_nickname) & player_nickname != "NULL" & nzchar(player_nickname),
        player_nickname,
        player_name
      )
    )

  if (!is.null(events_df) && nrow(events_df) > 0) {
  event_players <- events_df %>%
    filter(!is.na(player_id)) %>%
    transmute(
      player_id = as.integer(player_id),
      player_name = player_name,
      team_id = as.integer(team_id),
      team_name = team_name
    ) %>%
    distinct(player_id, .keep_all = TRUE)

  players <- players %>%
    full_join(event_players, by = "player_id", suffix = c("", "_event")) %>%
    mutate(
      player_name = coalesce(player_name, player_name_event),
      team_id = coalesce(team_id, team_id_event),
      team_name = coalesce(team_name, team_name_event),
      player_nickname = if_else(
        is.na(player_nickname) | player_nickname == "NULL",
        NA_character_,
        player_nickname
      ),
      player_display_name = coalesce(
        if_else(
          !is.na(player_nickname) & nzchar(player_nickname),
          player_nickname,
          NA_character_
        ),
        player_name
      )
    ) %>%
    select(player_id, player_name, player_nickname, player_display_name, team_id, team_name)
  }

  players
}

#' Build team lookup table
build_team_lookup <- function(lineups_df, events_df = NULL) {
  teams <- lineups_df %>%
    transmute(team_id, team_name) %>%
    distinct()

  if (!is.null(events_df) && nrow(events_df) > 0) {
    event_teams <- events_df %>%
      filter(!is.na(team_id)) %>%
      transmute(team_id = as.integer(team_id), team_name = team_name) %>%
      distinct()

    teams <- bind_rows(teams, event_teams) %>%
      distinct(team_id, .keep_all = TRUE)
  }

  teams
}

#' Apply consistent display names to events
apply_name_mapping <- function(events_df, players, teams) {
  events_df %>%
    left_join(
      players %>% select(player_id, player_display_name),
      by = "player_id"
    ) %>%
    left_join(
      teams %>% rename(team_name_mapped = team_name),
      by = "team_id"
    ) %>%
    mutate(
      player_display_name = coalesce(player_display_name, player_name),
      team_name = coalesce(team_name_mapped, team_name)
    ) %>%
    select(-team_name_mapped)
}
